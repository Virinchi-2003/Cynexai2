...
async function findVenkats() {
  try {
    const res = await client.execute("SELECT id, name, email, role, password_encrypted FROM users WHERE email LIKE '%venkat%' OR name LIKE '%Venkat%'");
    console.log("=== VENKAT USERS ===");
