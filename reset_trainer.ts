...
async function resetTrainer() {
  ...
  try {
    ...
    await client.execute({
      sql: "UPDATE users SET password_encrypted = ?, role = 'Teacher' WHERE email = 'trainer@cynexai.in'",
      args: [newEncrypted]
    });
    console.log("Successfully reset password and role for trainer@cynexai.in");
  } catch (e) {
    console.error(e);
  }
}
