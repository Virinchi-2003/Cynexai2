...
describe('Users API', () => {
  ...
  describe('getUsers', () => {
    it('should fetch users from database', async () => {
      ...
      expect(client.execute).toHaveBeenCalledWith({ sql: 'SELECT * FROM users', args: [] });
      expect(result).toEqual([{ id: '1', name: 'John Doe', role: 'Teacher', salary: 50000 }]);
    });
    ...
    it('should construct query with WHERE and ORDER BY when params provided', async () => {
      ...
      await getUsers({ role: 'Admin', status: 'Active' }, 'name', 'DESC');
      
      expect(client.execute).toHaveBeenCalledWith({
        sql: 'SELECT * FROM users WHERE role = ? AND status = ? ORDER BY name DESC',
        args: ['Admin', 'Active']
      });
    });
  });
